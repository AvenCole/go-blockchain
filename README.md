# go-blockchain

一个按课程大作业节奏逐步实现的 Go 区块链模拟仿真项目。

当前仓库的目标不是一次性把所有功能堆出来，而是按照阶段推进：

1. 先建立稳定的工程骨架
2. 再逐步实现区块链、交易、钱包、UTXO、共识、网络、GUI 等模块
3. 每个阶段同步补充可读、可讲解、可答辩的文档

## 当前进度

- Plan 1：项目初始化，已完成
- Plan 2：基础区块与区块链，已完成
- Plan 3：最简交易模型，已完成
- Plan 4：钱包系统，已完成
- Plan 5：UTXO 交易模型，已完成
- Plan 6：交易签名与验证，已完成
- Plan 7：UTXO 缓存与查询优化，已完成
- Plan 8：Merkle 树，已完成
- Plan 9：工作量证明共识，已完成
- Plan 10：交易池与经济模型，已完成
- Plan 11：网络模拟，已完成
- Plan 12：安全校验与攻击模拟，已完成
- Plan 13：GUI 演示层，已完成
- Plan 14：性能实验与优化，已完成
- Plan 15：实验报告与答辩材料整理，已完成
- Plan 16：Script / OP 虚拟机，已完成
- Plan 17：脚本可视化与 GUI 演示增强，已完成
- Plan 18：分叉处理与最长链切换，已完成
- Plan 19：链重组后的 Mempool 恢复策略，已完成
- Plan 20：GUI 网络运维台与内置命令控制台，已完成
- Plan 21：链切换 / 重组状态可视化，已完成
- Plan 22：最近链事件面板，已完成
- Plan 23：孤儿块缓冲与父块补取，已完成
- Plan 24：更细粒度的链事件类型，已完成
- Plan 25：更细粒度的网络事件观测，已完成
- Plan 26：第二种脚本模板 P2PK，已完成
- Plan 27：教学型多重签名，已完成
- Plan 28：GUI 脚本交易工作台，已完成
- Plan 29：GUI 多签花费向导，已完成
- Plan 30：Bitcoin Core 风格多区域工作台，已完成
- Plan 31：版本通告与链同步提示，已完成
- Plan 32：GUI 节点链控制与分布式演示，已完成
- Plan 33：GUI 一键网络演示场景，已完成
- Plan 34：GUI 分叉 / 重组一键演示，已完成
- Plan 35：三节点分区 / 合流重组演示，已完成
- Plan 36：网络拓扑与演示时间线可视化，已完成
- Plan 37：GUI 节点焦点面板与快捷操作，已完成
- Plan 38：GUI 控制台命令面板与预设工单，已完成
- Plan 39：网络事件筛选与搜索工作台，已完成
- Plan 40：网络流程进度反馈与运行锁，已完成
- Plan 41：前端路由现代化与多文件解耦重构，已完成

当前仓库建议作为**答辩封板版本**继续维护。
后续优先处理：

1. 实际答辩环境 smoke
2. 演示顺序和讲解整理
3. 阻塞性 bug 修复

## 仓库结构

```text
cmd/
  go-blockchain/        程序入口
internal/
  blockchain/           后续区块、区块链、交易主线
  cli/                  命令行入口
  config/               默认配置
  wallet/               后续钱包模块边界
  network/              后续网络模块边界
  gui/                  后续 GUI 模块边界
data/                   本地运行数据目录
docs/
  task.md               总体开发主流程
  plan/                 分阶段落地文档
tests/                  后续集成测试与验收脚本目录
```

## 当前可运行能力

当前仓库当前已支持的核心 CLI 能力包括：

- `createwallet`
- `listaddresses`
- `createblockchain`
- `send`
- `printmempool`
- `mine`
- `getbalance`
- `printchain`
- `showscript`
- `simdouble`
- `runperf`
- `startnode`
- `simfork`
- `simreorg`
- `showreorg`
- `showevents`
- `sendp2pk`
- `sendmultisig`
- `spendmultisig`

当前 GUI / Wails 演示层已支持：

- 钱包、区块、交易与脚本交易工作台
- GUI 内置终端执行 CLI 命令
- GUI 托管节点启动、连接、链初始化、节点发交易、节点挖矿
- 一键双节点网络同步演示
- 一键分叉 / 重组演示
- 一键三节点分区 / 合流收敛演示
- 网络拓扑总览与演示时间线可视化
- 节点目录、焦点面板与表单快捷填充
- 控制台命令预设、直接执行与最近命令复用
- 网络页事件筛选、来源过滤与关键字搜索
- 网络流程阶段进度反馈、运行状态卡与按钮锁定

示例：

```bash
go run ./cmd/go-blockchain createwallet
go run ./cmd/go-blockchain showscript <address>
go run ./cmd/go-blockchain createblockchain <miner-address>
go run ./cmd/go-blockchain printchain
```

GUI 默认使用独立数据目录：

- CLI / 节点：`./data`
- GUI：`./data/gui-desktop`

如需修改 GUI 数据目录，可设置环境变量：

- `GO_BLOCKCHAIN_GUI_DATA_DIR`

## 文档怎么读

### 1. `docs/task.md`

这是整个大作业的总流程文档，用来说明完整开发路线和阶段顺序。

### 2. `docs/plan/README.md`

这是分阶段文档体系说明，介绍每个计划文档应该怎么写、怎么维护。

### 3. `docs/plan/plan1.md`

解释项目初始化阶段为什么这样做、目录怎么设计、CLI 和配置层怎么落地。

### 4. `docs/plan/plan2.md`

解释基础区块链阶段应该实现什么、代码会落在哪些位置、关键数据结构和开发顺序是什么。

### 5. `docs/plan/plan3.md`

解释最简交易模型如何落地，包括交易结构、coinbase 原型交易、区块内交易存储、CLI 演示方式和当前简化边界。

### 6. `docs/plan/plan4.md`

解释钱包系统如何落地，包括密钥生成、地址生成、钱包持久化、多钱包管理和 CLI 使用方式。

### 7. `docs/plan/plan5.md`

解释 UTXO 交易模型如何落地，包括输入引用历史输出、找零逻辑、UTXO 扫描、余额计算方式和 CLI 演示流程。

### 8. `docs/plan/plan6.md`

解释交易签名与验证如何落地，包括签名副本、引用输出校验、签名验证流程以及 CLI 演示方式。

### 9. `docs/plan/plan7.md`

解释 UTXO 缓存与查询优化如何落地，包括缓存键设计、重建索引、增量更新和 CLI 演示方式。

### 10. `docs/plan/plan8.md`

解释 Merkle 树如何落地，包括交易哈希叶子、奇数节点补齐、MerkleRoot 写入区块头以及完整性校验方式。

### 11. `docs/plan/plan9.md`

解释工作量证明如何落地，包括 Nonce、Difficulty、目标值判断、挖矿循环和区块校验方式。

### 12. `docs/plan/plan10.md`

解释交易池与经济模型如何落地，包括 Mempool、手续费、coinbase 奖励、显式挖矿流程和 CLI 演示方式。

### 13. `docs/plan/plan11.md`

解释网络模拟如何落地，包括节点启动、节点发现、交易广播、区块广播和基础同步流程。

### 14. `docs/plan/plan12.md`

解释安全校验与攻击模拟如何落地，包括双花检测、非法交易拦截、非法区块拒绝和 CLI 攻击演示入口。

### 15. `docs/plan/plan13.md`

解释 GUI 演示层如何落地，包括 Wails 桌面壳、React 19 + TypeScript + MUI 前端、多页面结构和真实后端绑定。

### 16. `docs/plan/plan14.md`

解释性能实验与优化如何落地，包括 cache vs scan 对比、CLI 实验入口和结果输出文件。

### 17. `docs/perf/`

保存性能实验输出文件，便于答辩和实验报告直接引用。

### 18. `docs/plan/plan15.md`

解释实验报告、架构说明、答辩脚本和检查清单为什么要单独整理成正式交付物。

### 19. `docs/plan/plan16.md`

解释 Script / OP 虚拟机如何落地，包括最小 P2PKH 指令集、脚本执行流程、兼容旧交易的原因和 CLI 演示方式。

### 20. `docs/plan/plan17.md`

解释如何把脚本系统在 GUI 中可视化展示，并同步处理 GUI 数据库句柄的稳定性问题。

### 21. `docs/plan/plan18.md`

解释如何保存侧链块、在更长分叉出现时切换到最长链，并通过 CLI 演示这一过程。

### 22. `docs/plan/plan19.md`

解释链重组后如何恢复旧主链掉下来的合法交易，并同步清理已在新主链确认的池中交易。

### 23. `docs/plan/plan20.md`

解释如何把网络节点操作和 CLI 命令执行能力整合进 GUI 工作台。

### 24. `docs/plan/plan21.md`

解释如何持久化最近一次链重组状态，并在 CLI 与 GUI 中做可视化展示。

### 25. `docs/plan/plan22.md`

解释如何把最近链事件列表持久化，并在 CLI 与 GUI 中展示最近事件序列。

### 26. `docs/plan/plan23.md`

解释节点如何缓存孤儿块、补取父块，并在父块到达后自动重试导入。

### 27. `docs/plan/plan24.md`

解释如何把链事件类型从单一 reorg 扩展为 genesis / main_block / fork_block / reorg 等更完整的链行为序列。

### 28. `docs/plan/plan25.md`

解释如何记录节点最近网络事件，并在 GUI 网络页中展示这些事件。

### 29. `docs/plan/plan26.md`

解释如何在现有 Script VM 中加入第二种真实可用的脚本模板 P2PK。

### 30. `docs/plan/plan27.md`

解释如何在现有 Script VM 上继续加入教学型多重签名模板。

### 31. `docs/plan/plan28.md`

解释如何把不同脚本模板的交易构造能力直接整合进 GUI 交易页。

### 32. `docs/plan/plan29.md`

解释如何在 GUI 中直接选择并花费未花费多签输出。

### 33. `docs/plan/plan30.md`

解释如何把 Dashboard 重构为更接近 Bitcoin Core 的多区域总控台。

### 34. `docs/plan/plan31.md`

解释节点在挖矿或导入新区块推进 tip 后，如何主动向 peer 通告自己的最新高度。

### 35. `docs/plan/plan32.md`

解释如何在 GUI 中直接为指定节点初始化链、通过节点发送交易并触发节点挖矿。

### 36. `docs/plan/plan33.md`

解释如何把双节点同步、交易广播和挖矿封装成 GUI 一键演示场景。

### 37. `docs/plan/plan34.md`

解释如何把分叉 / 重组流程封装为 GUI 一键运行能力。

### 38. `docs/plan/plan35.md`

解释如何把三节点分区、合流与最终收敛封装成稳定可复现的网络流程。

### 39. `docs/plan/plan36.md`

解释如何把网络拓扑和统一事件时间线接入 GUI 网络页。

### 40. `docs/plan/plan37.md`

解释如何通过节点目录、焦点面板和快捷填充按钮，把 GUI 网络页进一步收敛成更好操作的工具工作台。

### 41. `docs/plan/plan38.md`

解释如何把 GUI 控制台升级为包含命令预设、直接执行和最近命令复用的命令工作台。

### 42. `docs/plan/plan39.md`

解释如何把网络页中的时间线、链事件和节点事件区域升级为可筛选、可搜索的观测工作台。

### 43. `docs/plan/plan40.md`

解释 Plan 40 如何把网络页长流程操作升级为具备进度反馈、失败定位和运行锁的工具工作台。

### 44. `docs/plan/plan41.md`

解释如何把 GUI 顶层单文件结构重构为 TanStack Router + 多文件分层前端架构。

### 45. `docs/defense-script.md`

记录当前封板版本推荐的答辩演示流程、讲解稿、压缩版演示顺序与现场问答。

### 46. `docs/demo-checklist.md`

记录答辩前环境、GUI、CLI、网络页和材料准备的检查清单。

### 47. `docs/final-validation.md`

记录当前答辩基线已经执行过的自动化验证命令与建议的手动 smoke 路径。

## 开发原则

1. 每个阶段先明确边界，再落地实现。
2. 每次开发时同步更新 `docs/plan/`，不是最后补文档。
3. 小更改直接提交并 push。
4. 大更改走 PR，并完成审核后合并。
5. 先打通命令行主链路，再扩展 GUI 和更复杂功能。

## 下一步

当前已完成到 Plan 40：网络流程进度反馈与运行锁。

当前建议把仓库作为答辩基线先冻结，不再主动继续扩展新功能。
完整验证路径请看：

- `docs/final-validation.md`

当前新增的关键能力包括：

1. `scriptSig` / `scriptPubKey`
2. 最小 P2PKH Script VM
3. `showscript` CLI 演示入口
4. 新交易走脚本执行验证，同时兼容旧交易
5. GUI 钱包/区块页面可直接展示脚本信息
6. 更长分叉出现时可自动切换到最长链
7. 链重组后会恢复仍然合法的旧主链交易到 Mempool
8. GUI 已支持网络节点管理和内置 CLI 控制台
9. CLI / GUI 已支持最近一次链重组状态展示
10. CLI / GUI 已支持最近链事件列表展示
11. 节点已支持孤儿块缓冲与自动补链重试
12. 最近链事件已覆盖 genesis / main_block / fork_block / reorg
13. GUI 网络页已支持节点最近网络事件观测
14. Script VM 已支持 P2PKH + P2PK 两种脚本模板
15. Script VM 已支持教学型多重签名模板
16. GUI 交易页已支持选择脚本模板并发起脚本交易
17. GUI 已支持直接选择并花费未花费多签输出
18. Dashboard 已重构为多区域总控台，可同时看到链、钱包、Mempool、多签与节点活动摘要
19. 节点在 tip 前进后会主动发出高度通告
20. GUI 网络页可直接初始化节点链、通过节点发交易并触发节点挖矿
21. GUI 网络页和控制台都可一键跑通双节点同步演示
22. GUI 网络页已支持节点目录、焦点面板与地址快捷填充
23. GUI 控制台已支持常用命令预设、直接执行与最近命令复用
24. GUI 网络页已支持时间线、链事件和节点事件筛选与搜索
25. GUI 网络页已支持长流程进度反馈、失败定位和运行锁

后续如果继续扩展，优先方向会是：

1. 更真实的 orphan / reorg 广播策略
2. 更细粒度的脚本与网络联合演示面板
3. 更复杂的脚本策略组合
4. 更接近正式答辩材料的自动化生成
